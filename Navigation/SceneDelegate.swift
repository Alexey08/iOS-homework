...
class SceneDelegate: UIResponder, UIWindowSceneDelegate {
    ...
    func scene(_ scene: UIScene, willConnectTo session: UISceneSession, options connectionOptions: UIScene.ConnectionOptions) {
        ...
        profileVC.tabBarItem = UITabBarItem(title: "Profile", image: .remove, tag: 0)
        let profileNavC = UINavigationController(rootViewController: profileVC)
        profileNavC.navigationBar.backgroundColor = .white
        
        let feedVC = FeedViewController()
        feedVC.tabBarItem = UITabBarItem(title: "Feed", image: .add, tag: 1)
        let feedNavC = UINavigationController(rootViewController: feedVC)
        feedNavC.navigationBar.backgroundColor = .white

        tabBarControler.viewControllers = [profileNavC, feedNavC]
